...
if [[ "${MUTAGEN_OS_NAME}" == "darwin" ]]; then
    ...
    /usr/bin/ditto -c -k --keepParent build/compose/darwin_amd64 "${RUNNER_TEMP}/notarize_compose_darwin_amd64.zip"
    /usr/bin/ditto -c -k --keepParent build/compose/darwin_arm64 "${RUNNER_TEMP}/notarize_compose_darwin_arm64.zip"
