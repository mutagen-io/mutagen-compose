...
if [[ "${MUTAGEN_OS_NAME}" == "darwin" ]]; then
    ...
    MUTAGEN_VERSION="$(go run scripts/ci/mutagen_version.go)"

    # Convert the windows/386 bundle to zip format.
    tar xzf "build/release/mutagen-compose_windows_386_v${MUTAGEN_VERSION}.tar.gz"
    zip "build/release/mutagen-compose_windows_386_v${MUTAGEN_VERSION}.zip" mutagen-compose.exe
    rm mutagen-compose.exe

    # Convert the windows/amd64 bundle to zip format.
    tar xzf "build/release/mutagen-compose_windows_amd64_v${MUTAGEN_VERSION}.tar.gz"
    zip "build/release/mutagen-compose_windows_amd64_v${MUTAGEN_VERSION}.zip" mutagen-compose.exe
    rm mutagen-compose.exe

    # Convert the windows/arm bundle to zip format.
    tar xzf "build/release/mutagen-compose_windows_arm_v${MUTAGEN_VERSION}.tar.gz"
    zip "build/release/mutagen-compose_windows_arm_v${MUTAGEN_VERSION}.zip" mutagen-compose.exe
    rm mutagen-compose.exe

    # Convert the windows/arm64 bundle to zip format.
    tar xzf "build/release/mutagen-compose_windows_arm64_v${MUTAGEN_VERSION}.tar.gz"
